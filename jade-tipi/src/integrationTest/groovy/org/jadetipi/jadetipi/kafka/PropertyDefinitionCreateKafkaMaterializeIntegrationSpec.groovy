...
class PropertyDefinitionCreateKafkaMaterializeIntegrationSpec extends Specification {
    ...
    def 'open + ppy-definition + ppy-assignment + commit materializes the definition root and leaves the assignment skipped'() {
        given: 'one canonical property-definition transaction with a trailing assignment that should remain skipped'
        ...
        Message commitMsg = Message.newInstance(txn, JtpCollection.TRANSACTION, Action.COMMIT, [
                summary: 'one property-definition created; one assignment intentionally still skipped'
        ])
        ...
        and: 'the trailing ppy assignment message is intentionally still not materialized in the ppy collection'
