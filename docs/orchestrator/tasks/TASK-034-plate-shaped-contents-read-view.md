...
- Return a fixed 96-well view with row labels `A` through `H`, columns `1`
  through `12`, and row-major `wells` output.
...
- Preserve links that cannot be placed under `unplacedContents`; do not drop
  invalid, missing, or out-of-range position data.
...
- No lookup or validation of the container `loc` root.
