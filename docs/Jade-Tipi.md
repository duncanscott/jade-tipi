...
Groups are managed at an institutional level and play a key role in ensuring that IDs are world-unique.  A group record is itself a Jade-Tipi object in the `grp` collection, with a world-unique ID, properties, and possible links.  The group record may also declare access granted to other groups, using a simple permissions map where each referenced group receives either read/write (`rw`) or read-only (`r`) access to objects owned by the group.  Finer-grained overrides on individual objects or property-value assignments may be useful later, but should be introduced only after concrete use cases justify the added complexity.
